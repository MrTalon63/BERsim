...
RESULT_QUEUE_DIR = os.environ.get(
    "RESULT_QUEUE_DIR", os.path.join(os.getcwd(), "failed_results")
)
...
def submit_with_retries(
    ...
    endpoint,
    payload,
    max_retries=RESULT_RETRY_MAX,
    backoff_factor=RESULT_RETRY_BACKOFF,
    timeout=10,
):
    attempt = 0
    while True:
        try:
            ...
            if attempt > max_retries:
                print(
                    f"submit_with_retries: final failure after {max_retries} attempts: {exc}"
                )
                return False
            ...
            sleep_time += jitter
            print(
                f"submit_with_retries: attempt {attempt}/{max_retries} failed, sleeping {sleep_time:.1f}s"
            )
...
def main():
    ...
    def find_simulator():
        ...
        if sim_env:
            ...
            if which_env:
                print(
                    f"Using simulator from PATH resolving SIMULATOR_PATH: {which_env}"
                )
                return which_env
            print(
                f"SIMULATOR_PATH set but not found/executable: {sim_env}; will try fallback locations."
            )
    ...
    # Start background flush worker and attempt immediate flush
    ...
    while not stop:
        try:
            ...
            print(
                f"Cannot reach Master server or HTTP error: {e}. Retrying in 5 seconds..."
            )
            time.sleep(5)
            continue
        ...
        if data.get("status") == "wait":
            print(
                f"\rNo task available. Slave [{hostname}] waiting...",
                end="",
                flush=True,
            )
            continue
        ...
        ebno = task["ebno"]
        max_frames = data.get(
            "max_frames", os.environ.get("SIM_MAX_FRAMES", "10000000")
        )
        print(
            ...
            if char in ("\r", "\n"):
                ...
                if "~> Time:" in line and "Frames:" in line:
                    ...
                    try:
                        parts = line.split("|")
                        frames = int(
                            [p for p in parts if "Frames:" in p][0]
                            .split(":")[1]
                            .split("/")[0]
                            .strip()
                        )
                        fe = int(
                            [p for p in parts if "FE:" in p][0]
                            .split(":")[1]
                            .split("/")[0]
                            .strip()
                        )
                                ...
                                if cancel_check.status_code == 200:
                                    ...
                                    if cancel_data.get("cancelled"):
                                        ...
                                        try:
                                            ...
                                            print(
                                                "[CANCEL] Subprocess did not terminate; killing it."
                                            )
                                            proc.kill()
                                            proc.wait()
                                        print(
                                            f"[CANCEL] Task cancelled. Requesting new task..."
                                        )
                                        break
                            except Exception:
                                pass
                    ...
                    if "|" in line and "Time:" not in line and "Eb/No" not in line:
                        ...
                        try:
                            if (
                                len(parts) >= 5
                                and sys_name in parts[0]
                                and str(ebno) in parts[1]
                            ):
                                result_ber, result_fer, result_frames = (
                                    float(parts[2]),
                                    float(parts[3]),
                                    int(parts[4]),
                                )
        ...
        if proc.returncode != 0:
            ...
            success = submit_with_retries(
                session, f"{MASTER_URL}/submit_result", payload, timeout=10
            )
            if not success:
                print(
                    "Failed to POST results to Master after retries; queuing result for later retry."
                )
